...
class OptionsManager:
    ...
    def setup_input_tab(self):
        ...
        self.contract_size_entry.grid(row=6, column=1, sticky='ew', pady=2)
        ...
        self.contract_size_helper = ttk.Label(fields_frame, text="(100 for options, 1 for stocks)", font=('Arial', 8),
                                              foreground='gray')
        self.contract_size_helper.grid(row=6, column=2, columnspan=2, sticky='w', pady=2, padx=(10, 0))

        # Expiration date
        ttk.Label(fields_frame, text="Expiration Date (YYYY-MM-DD):").grid(row=7, column=0, sticky='w', pady=2)
        self.expiry_var = tk.StringVar()
        self.expiry_entry = ttk.Entry(fields_frame, textvariable=self.expiry_var)
        self.expiry_entry.grid(row=7, column=1, sticky='ew', pady=2)
        self.expiry_entry.bind('<KeyRelease>', self.calculate_iv)

        # Risk-free rate
        ttk.Label(fields_frame, text="Risk-free Rate (%):").grid(row=8, column=0, sticky='w', pady=2)
        self.rate_var = tk.DoubleVar(value=5.0)
        self.rate_entry = ttk.Entry(fields_frame, textvariable=self.rate_var)
        self.rate_entry.grid(row=8, column=1, sticky='ew', pady=2)
        self.rate_entry.bind('<KeyRelease>', self.calculate_iv)

        # Dividend yield
        ttk.Label(fields_frame, text="Dividend Yield (%):").grid(row=9, column=0, sticky='w', pady=2)
        self.dividend_var = tk.DoubleVar(value=0.0)
        self.dividend_entry = ttk.Entry(fields_frame, textvariable=self.dividend_var)
        self.dividend_entry.grid(row=9, column=1, sticky='ew', pady=2)
    ...
    def setup_positions_tab(self):
        ...
        columns = (
        'Trade', 'Type', 'Strike', 'Contracts', 'Open Price', 'Days Left', 'IV%', 'Premium', 'Risk Up', 'Risk Down')
    ...
    def setup_closed_tab(self):
        ...
        columns = ('Type', 'Strike', 'Contracts', 'Open Price', 'Close Price', 'Win/Loss')
        self.closed_tree = ttk.Treeview(self.closed_frame, columns=columns, show='headings', height=15)
        ...
        # Scrollbar
        closed_scrollbar = ttk.Scrollbar(self.closed_frame, orient='vertical', command=self.closed_tree.yview)
        self.closed_tree.configure(yscrollcommand=closed_scrollbar.set)

        self.closed_tree.pack(side='left', fill='both', expand=True, padx=10, pady=10)
        closed_scrollbar.pack(side='right', fill='y', pady=10)
    ...
    def calculate_risk(self, position):
        """Calculate risk for up and down movements using intrinsic values"""
    ...
    def calculate_premium(self, position):
        """Calculate premium based on new requirements"""
        option_type = position['option_type']
    ...
    def close_position(self):
        ...
        close_price = tk.simpledialog.askfloat("Close Position",
                                               f"Enter close price for {position['option_type']} ${position['strike']:.2f}:")
    ...
    def show_portfolio_graph(self):
        """Show portfolio value graph across underlying price range with both pricing methods"""
        ...
        try:
            ...
            for price in price_range:
                ...
                for pos in self.positions:
                    ...
                    if pos['option_type'] == 'Stock':
                        # For stocks, Black-Scholes value is the same as intrinsic value
                        ...
                        bs_value = self.black_scholes(
                            ...
                            pos['rate'],
                            pos['dividend'],
                            pos['iv'] / 100,  # Convert IV back to decimal
                            pos['option_type']
                        )
            ...
            graph_window.title("Portfolio Value Graph - Both Methods")
            graph_window.geometry("900x700")

            # Create matplotlib figure
            fig, ax = plt.subplots(figsize=(12, 8))

            ax.plot(price_range, portfolio_values_intrinsic, 'b-', linewidth=2, label='Intrinsic Value')
            ...
            ax.axvline(x=current_price, color='g', linestyle='--', alpha=0.7,
                       label=f'Current Price (${current_price:.2f})')

            ax.set_xlabel('Underlying Price ($)')
            ax.set_ylabel('Portfolio Value ($)')
            ax.set_title('Portfolio Value vs Underlying Price (Intrinsic vs Black-Scholes)')
